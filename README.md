...
The project is in **M5 — Maps & content** (in progress; P01–P15 in).
...
- [x] **P15** — ComfyUI asset-generation pipeline + first approved Trooper atlas (`assets/sprites/trooper.png`). New `tools/comfy/` directory with `asset.py` orchestrator (skeleton → anchor → canonical → crop → install → verify), `run_workflow.py` ComfyUI API driver, `crop_canonical.py` (slice canonical → atlas + Pipeline 7 post: alpha-key + Bayer-8 halftone + 2-color palette remap + nearest-point round-trip), `transcribe_atlas.py` (C-literal generator), `make_trooper_skeleton.py` (programmatic full-frame mech anatomy with helmet+visor / shoulder pads / panel-lined torso / gauntlets / boots / 16 magenta pivot dots). Two workflow JSONs: `mech_chassis_canonical_v1.json` (spec — SDXL + ControlNet-Union promax + IP-Adapter PLUS, for 12GB+ GPUs) and `mech_chassis_canonical_8gb_v1.json` (working subset for RTX 2080 / 8GB — T2I-Adapter Lineart substitute, no IP-Adapter, CPU VAE; ~45 sec end-to-end). Style anchor `trooper_anchor_v2.png` is itself AI-generated (best-of-4 from the prompt-only `style_anchor_v1.json` workflow at seed 7700001 — TRO-style ink mech with halftone fill). Plus two render-side sprite-anchor fixes: `draw_mech_sprites` scales 2-particle bone sprites to actual bone length (was authored 2.4–5.7× larger, dense AI tiles overhung past particles); foot pivot moved from sprite-center to bottom-edge so the boot draws above the FOOT particle (which is the ground-contact point — center pivot was clipping the boot 12 px into the platform). New trade-off entry "ControlNet-Union promax + IP-Adapter chain doesn't fit in 8GB VRAM" documents the hardware ceiling and the substitute that does fit. `assets/credits.txt` + `documents/art_log.md` updated with the approved Trooper row.
- [ ] **P16** — Generate the remaining 4 chassis (Scout / Heavy / Sniper / Engineer) + weapon atlas + HUD icons + per-map parallax kits via the same pipeline
