...
- [ ] **P16** — Remaining 4 chassis (Scout / Heavy / Sniper / Engineer) + weapon atlas + HUD icons + per-map parallax kits, chassis via the gostek part-sheet path (one `<chassis>_gostek_v*.png` per chassis → `tools/comfy/extract_gostek.py` → `assets/sprites/<chassis>.png`; per-chassis bone proportions per `documents/m5/12-rigging-and-damage.md`); the AI-diffusion pipeline stays available as a fallback
