...
/* ---- Map-thumbnail cache -------------------------------------------
 *
 * The vote picker on the summary screen shows three map cards. Each
 * card's preview is a 256×144 PNG generated by `tools/cook_maps` at
 * bake time and shipped under `assets/maps/<short>_thumb.png`. We
 * lazy-load on first reference (same pattern as `g_decorations_atlas`
 * / `g_hud_atlas` in render.c) and cache one texture per registry
 * slot — a map's thumb persists for the process lifetime. Custom maps
 * without a paired _thumb.png fall back to the gray-rect placeholder.
 */
static Texture2D g_map_thumbs[MAP_REGISTRY_MAX];
static bool      g_map_thumb_attempted[MAP_REGISTRY_MAX];

void lobby_ui_clear_thumb_cache(void) {
    for (int i = 0; i < MAP_REGISTRY_MAX; ++i) {
        if (g_map_thumbs[i].id != 0) UnloadTexture(g_map_thumbs[i]);
        g_map_thumbs[i] = (Texture2D){0};
        g_map_thumb_attempted[i] = false;
    }
}

/* Decode raylib-format PNG bytes into a texture and cache. */
static Texture2D map_thumb_from_png_bytes(const uint8_t *png, int png_size) {
    if (!png || png_size <= 0) return (Texture2D){0};
    Image img = LoadImageFromMemory(".png", png, png_size);
    if (img.data == NULL) return (Texture2D){0};
    Texture2D t = LoadTextureFromImage(img);
    UnloadImage(img);
    if (t.id == 0) return (Texture2D){0};
    SetTextureFilter(t, TEXTURE_FILTER_BILINEAR);
    return t;
}

/* Pull the THMB lump out of whatever .lvl on disk corresponds to this
 * MapDef. Host's own + bundled-asset maps land in `assets/maps/`;
 * downloaded maps live in the content-addressed cache. */
static Texture2D map_thumb_from_lvl(const MapDef *md) {
    char path[1024];
    snprintf(path, sizeof path, "assets/maps/%s.lvl", md->short_name);
    if (!FileExists(path) && md->file_crc != 0) {
        const char *cached = map_cache_path(md->file_crc);
        if (cached && cached[0] && FileExists(cached)) {
            snprintf(path, sizeof path, "%s", cached);
        } else {
            return (Texture2D){0};
        }
    ...
    int sz = 0;
    uint8_t *png = level_io_read_thumb(path, &sz);
    if (!png) return (Texture2D){0};
    Texture2D t = map_thumb_from_png_bytes(png, sz);
    free(png);
    return t;
}

static Texture2D map_thumb_get(int map_id) {
    if (map_id < 0 || map_id >= MAP_REGISTRY_MAX) return (Texture2D){0};
    if (g_map_thumb_attempted[map_id]) return g_map_thumbs[map_id];
    g_map_thumb_attempted[map_id] = true;
    const MapDef *md = map_def(map_id);
    if (!md || md->short_name[0] == 0) return (Texture2D){0};

    /* Sidecar fast path — shipped assets + editor saves drop one
     * next to the .lvl. */
    char png_path[256];
    snprintf(png_path, sizeof png_path,
             "assets/maps/%s_thumb.png", md->short_name);
    if (FileExists(png_path)) {
        Texture2D t = LoadTexture(png_path);
        if (t.id != 0) {
            SetTextureFilter(t, TEXTURE_FILTER_BILINEAR);
            g_map_thumbs[map_id] = t;
            return t;
        }
        LOG_W("lobby: LoadTexture(%s) failed", png_path);
    }

    /* Fallback — extract the THMB lump from the .lvl itself. This is
     * the wire path: clients that downloaded a custom map don't have
     * the sidecar PNG, but the .lvl carries the thumb bytes inline. */
    Texture2D t = map_thumb_from_lvl(md);
    if (t.id != 0) {
        g_map_thumbs[map_id] = t;
        return t;
    }

    LOG_I("lobby: no thumb for %s; placeholder only", md->short_name);
    return (Texture2D){0};
}
...
void summary_screen_run(LobbyUIState *L, Game *g, int sw, int sh) {
    ...
    /* Panel is tall enough to host a 16:9 thumbnail (S(112)) above
     * the name + blurb + Vote button. S(180) — the pre-thumb size —
     * forced the thumb into a 200×64 strip that mangled the source
     * aspect; bumping to S(240) lets the picture breathe. */
    int  vote_panel_h   = S(240);
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            DrawRectangleLinesEx(cr, L->ui.scale,
                                 mine ? (Color){180, 230, 130, 255}
                                      : L->ui.panel_edge);
            /* Map thumbnail. 16:9 strip (S(200)×S(112)) sourced from
             * assets/maps/<short>_thumb.png — cooked by
             * tools/cook_maps. Falls back to the dark-gray rect when
             * the PNG is missing (custom maps without a paired
             * thumb, or a stripped install). */
            Rectangle thr = (Rectangle){
                cr.x + S(10), cr.y + S(10),
                cr.width - 2 * S(10), S(112)
            };
            DrawRectangleRec(thr, (Color){50, 56, 70, 255});
            Texture2D thumb = map_thumb_get(candidates[i]);
            if (thumb.id != 0) {
                Rectangle src = (Rectangle){
                    0, 0, (float)thumb.width, (float)thumb.height
                };
                DrawTexturePro(thumb, src, thr, (Vector2){0, 0}, 0.0f,
