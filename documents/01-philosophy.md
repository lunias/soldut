...
4. **Threads added deliberately, not as a performance hammer.** The original "no threads in v1" rule was relaxed at M5 wan-fixes-16 for one specific case: the Host UI's dedicated server runs on a separate thread of the host process because Windows silently drops UDP between processes that share a spawn-tree ancestor (proven across two machines after five spawn-isolation workarounds failed). Two `Game` structs, zero shared mutable state, communication strictly over the kernel UDP socket pair — same shape we'd use for a future network thread or audio thread. Any further thread additions must clear the same bar: explicit message queues, no shared mutable state, no fixing CPU-bound code by parallelizing it. See `TRADE_OFFS.md` → "Host UI's dedicated server runs in a thread, not a child process".
