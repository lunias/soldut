...
> **Production-path note (P15 revised, 2026-05-10).** The *look* this document specifies is unchanged — but the **per-chassis mech sprite atlases** (`assets/sprites/<chassis>.png`) are no longer produced by "Pipeline 1" (AI-diffusion canonical → 17-tile crop). That path plateaued below the bar (Illustrious-XL is an anime base; IP-Adapter style-transfer can't override the prior; the BattleTech-Battlemechs LoRA named below is SD1.5-architecture and won't load on SDXL; a detailed-illustration crop renders as same-colour mush at the ~80 px the mech occupies on screen, because the 17-part rigid renderer wants clean flat plates). The shipping path is now `tools/comfy/extract_gostek.py` — it slices a **Soldat-style "gostek part sheet"** (one image with the 22 body parts laid out flat-shaded + black-outlined + captioned in reading order) straight into the atlas. See `tools/comfy/README.md` → "The shipping path", `TRADE_OFFS.md` → "Chassis art is hand-authored gostek part sheets, not the AI-diffusion pipeline", and `documents/m5/prompts/P16-comfyui-rest.md`. Pipelines 2–8 below (parallax, decorations, weapon atlas, HUD icons, stump caps, post-process) and the *aesthetic* (2-colour print register, hard outlines, halftone screen at runtime) still apply as written; Pipeline 1 stays here as the documented alternative chassis path.
