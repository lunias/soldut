...
- [x] **P05 — Bot AI improvements** (shipped 2026-05-13; see `documents/m6/05-bot-ai-improvements.md`). Per-node visibility precompute (`BotNav.vis_bits[]`, sub-3 ms build per map), cover-aware engagement positioning, weapon-range gating, aggression-based retreat, CTF team-role coordination (attacker / defender / floater / carrier), Mass Driver balance retune. 1v1 PASS rate 11/32 → 26/32 cells (2.4×). 4 of 8 maps got geometry edits to match the bot's pathing capabilities (Reactor archway, Catwalk elevated ramps, Crossfire shorter stubs, Citadel partition passage).
- [x] **P06 — Perf profiling & baseline** (shipped 2026-05-14; see `documents/m6/06-perf-profiling-and-optimization.md` + `documents/m6/perf-baseline.md`). New always-on `src/profile.{c,h}` frame profiler + `--bench` / `--bench-csv` CLI flags + `make perf-bench{,-uncapped,-stress,-flamegraph}` targets. Phase 4 prototyped H10a (collapse halftone + upscale into one backbuffer pass) but the WSL win was 0.3 ms (within noise) and it broke shot-test byte-identicality, reverted. Actionable finding: "30 FPS dips in WSL" is a WSLg compositor-pacing tax (15.5 ms `PROF_PRESENT`), not a code-side budget breach. Windows native hits 100 FPS in the same scenario.
- [x] **Lobby loadout preview** (shipped 2026-05-15; see commit `b9a6063`). New modal loadout picker — single `[ CHOOSE LOADOUT ]` button + treadmill animation cycle (8 phases × 16 s loop) + 6-axis radar chart normalised across all 2160 possible loadouts + per-jetpack exhaust FX matching `g_jet_fx`. Lives in `src/loadout_preview.{c,h}`.
- [x] **UI fixes** (shipped 2026-05-15; see commit `b514e04`). Title spacing, SFX stacking caps, lobby map-cycle direction arrows, TTF font Unicode coverage for em-dash / middle-dot, refreshed Controls modal.
- [x] **CTF fixes — round-shape redesign** (shipped 2026-05-15; this branch). User-reported CTF playability bugs unblocked, then round shape unified across all modes. Five commits: flag-pickup touch box (8b3894c), CTF capture +1 not +5 (0da627a), live HUD score banner sync (ea4c5d6), universal mid-round respawn + `rounds_per_match` host UI (43df2a4), per-round slot reset (95513ce). Defaults: score_limit=5 across FFA/TDM/CTF, rounds_per_match=3. New paired tests: `run_ctf_pickup`, `run_ctf_respawn`, `run_ctf_respawn_clean`, `run_ctf_score_limit`, `run_bot_team_sync`, `run_ffa_respawn`, `run_tdm_respawn`, `run_rounds_match`, `run_multi_round` (all green).
- [ ] **P07 — Movement gamefeel** (planned; `documents/m6/07-movement-gamefeel.md`)
