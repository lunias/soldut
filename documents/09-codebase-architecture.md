...
All modules expected by the design canon now ship — `audio.{c,h}` landed at M5 P14. Two additional modules landed off-roadmap with wan-fixes (2026-05-11): `prefs.{c,h}` for per-user loadout/name/team persistence, and `proc_spawn.{c,h}` for cross-platform process utilities used by the paired-process test harnesses. (`proc_spawn.{c,h}` originally backed a spawn-child "Host Server" architecture at wan-fixes-5/9; wan-fixes-16 replaced that with an in-process server thread after a Windows UDP-filter bug, so the spawn-child machinery is gone but the basic process utilities stay.)
...
| simulate.c | 310 | the pure step (P03 render_prev snapshot, P05 pickup_step hook, P12 per-mech `hit_flash_timer` decay + per-limb smoke-threshold check on every 8th tick) |
| physics.c | 753 | Verlet + constraints + tile/poly collision (M5 P02 grew this; P06 added `solve_fixed_anchor`) |
| mech.c | 2084 | chassis + animation drive — past split threshold (P05 powerups + Engineer deployable + Burst SMG cadence; P06 grapple lifecycle + ATTACHED retract block; P07 carrier penalties + `ctf_drop_on_death` hook in `mech_kill`; P09 `fire_other_slot_one_shot` RMB one-shot dispatch; P10 per-chassis bone-length distinctness in `g_chassis[]` + per-chassis posture quirks in `build_pose`; P11 `last_fired_slot` / `last_fired_tick` fields + grapple-branch migration to `weapon_muzzle_world(...)` — foregrip pose driver attempted in `build_pose` and reverted post-ship; P12 `hit_flash_timer` decay + per-limb `damage_decals` ring writes from `mech_apply_damage` + `mech_dismember` 64-particle radial spray + `fx_spawn_stump_emitter` arm + per-limb smoke-threshold check) |
...
| particle.c | 282 | pool + draw (P12 added `FX_STUMP` pinned emitter + `fx_spawn_stump_emitter` + tick-time parent-particle lookup + `fx_spawn_smoke` for damaged-limb cue) |
...
grapple branches). P12 grew `render.c` (+~155 LOC for `apply_hit_flash`
+ `draw_damage_decals` + `draw_stump_caps` + `part_local_to_world`),
`mech.c` (+~85 LOC for hit-flash decay + per-limb decal-ring writes +
`mech_dismember` heavy-spray + `fx_spawn_stump_emitter` arm + smoke
threshold), `mech_sprites.c` (+~55 LOC for the PART_* ↔ sprite-id
helpers), `particle.c` (+~92 LOC for `FX_STUMP` + `fx_spawn_smoke`),
and `simulate.c` (+~60 LOC for hit-flash decay + smoke-threshold tick).
The extraction (e.g., `net_lobby.c` / `net_map_share.c`;
`lobby_ui_match.c` / `lobby_ui_summary.c`) is worth scheduling before
the audio module (M5 P14) adds its own surface to `net.c`. `audio.{c,h}`
is not yet built — it lands at M5 P14 and will add to this table.
