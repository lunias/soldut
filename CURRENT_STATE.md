...
- `SetExitKey(KEY_NULL)` added to `platform_init` — every `IsKeyPressed(KEY_ESCAPE)` consumer is already explicit, so this fixes the latent bug where the title-screen Controls modal's ESC handler set `show_keybinds=false` but raylib's default ESC-to-close fired the same frame and exited the game anyway. Now the title-Controls / loadout-preview / in-match-end-round handlers all work as written.
- Lobby right column lost ~5 row heights of vertical real estate (chassis/primary/secondary/armor/jetpack cycles → one summary panel + one CHOOSE LOADOUT button). The chat box on the left grows into the freed space at the bottom of the lobby.
- Choice arrays (`g_chassis_choices`, `g_primary_choices`, `g_secondary_choices`, `g_armor_choices`, `g_jet_choices`) moved from `src/lobby_ui.c` to `src/loadout_preview.c`. `step_in_cycle` + `next_in_cycle` stayed in `lobby_ui.c` as unused functions (gated by `-Wno-unused-function`) — leaving them in place for now in case a future feature needs them.
- Em-dashes (—) and middle dots (·) in user-facing strings drawn through `ui_draw_text` render as the fallback `?` because Atkinson body / VG5000 display fonts load via `LoadFontEx(path, px, NULL, 0)` (the 95-glyph ASCII default). Existing lobby strings (`score 25 · time 600s`) live with this; new preview blurbs use ASCII " - " and " / " to read cleanly.

Previously: **2026-05-14** (M6 P06 — perf profiling & baseline). All five phases of `documents/m6/06-perf-profiling-and-optimization.md` walked on `m6-perf-profiling`; the shipped artifacts are the **always-on `src/profile.{c,h}` frame profiler** + `--bench` / `--bench-csv` CLI flags + `make perf-bench{,-uncapped,-stress,-flamegraph}` targets + `tools/perf/flamegraph.sh` + the captured baseline at `documents/m6/perf-baseline.md`. Phase 4 prototyped H10a (collapse halftone shader + upscale into one backbuffer pass, drop `g_post_target`) but the measured WSL win was 0.3 ms (within noise — the WSLg compositor stall in `PROF_PRESENT` absorbed what `PROF_DRAW_BLIT` was paying) AND it broke shot-test byte-identicality, so H10a was reverted.
...
- **Phase 2 — Cover-aware engagement positioning.** New `nav_pick_position(from, target_node, BOT_POS_ENGAGE|COVER|FLANK, max_walk, optimal_range, prefers_high)` — flat scan over all nav nodes, scored by visible-to-target predicate + weapon-stand-off proximity. `tactic_engage` now refreshes a per-mind `engagement_node` cache (`BotMind.engagement_node` + `..._tick`, refresh every 2 s or on enemy change) and plans an A* path to the engagement node when the enemy is LOS-blocked. `score_engage` no longer requires LOS — it accepts LOS-blocked enemies at a 0.85× damp so the goal still triggers and Phase 2 reposition fires. `score_pursue_enemy` also routes via an engagement node (not the enemy's pelvis-nearest), so cross-map pursuit chains through nodes WITH LOS to the enemy.

- **Phase 1/2 collateral — nav graph fundamentals.** Several reach-classifier rules tightened in the same pass because Phase 2's BFS exposed nav graph disconnects:
  - **WALK slope tolerance.** Pre-P05 WALK required `|dy| ≤ 8`. New rule allows `|dy| ≤ 1.2 × |dx|` up to 120 px (~50° slope), so 30° / 49° slope polys (Reactor bowl, Concourse hills, Citadel ramparts) connect across adjacent floor samples. Corridor check skipped for adjacent SLOPE pairs (the ray at -8/-40 clips into the slope poly itself when descending); flat-ground WALK still runs the corridor check so wall stubs between sample columns (Concourse cover columns) still cull through-wall edges.
  - **JET arc feasibility.** Pre-P05 JET used a single straight-line LOS ray that false-rejected any arc UP-AND-OVER an obstacle (Reactor pillar overhangs blocked every flank → pillar-top jet). New `jet_arc_clear()` casts two segments through an iterative peak (200 → 800 px above the higher endpoint in 100-px steps); succeeds if any peak height clears.
  - **JET reach prune** bumped 280 → 700 px (Reactor flank-to-pillar = 576 px). **JET vertical** bumped 200 → 360 px. **GRAPPLE prune** bumped 300 → 400 px. **Mid-range FALL** added (close-range FALL was capped by the 280-px prune; pillar-top → bowl-floor descents need 320+ px FALL).
  - **Pickup/spawn nodes get `BOT_NODE_F_ON_FLOOR`** after the snap-to-floor pass. Pre-P05 they had only `_PICKUP` / `_SPAWN`, so the WALK classifier's "both on floor" precondition rejected every pickup→floor adjacency, fragmenting big maps.
  - **A\* expansion budget** bumped 4N → 12N.
  - **`bfs_closest_reachable` fallback.** When direct A* fails (goal in a disconnected component), the bot picks the most goal-proximate REACHABLE node from start and paths there — so the bot still makes progress instead of pushing into geometry.

- **Phase 3 — Weapon-range gating + per-weapon engagement style.** New `WeaponEngagementProfile g_weapon_profiles[WEAPON_COUNT]` table — per-weapon optimal_range_px / effective_range_px / ideal_strafe_px / prefers_high. `weapon_range_fit_factor(weapon_id, dist)` is a 0..1 multiplier: full inside optimal range, linear taper to 0.5 at effective range, 0.15 past. Three call sites:
  1. `score_engage` multiplies the score by `weapon_range_fit_factor` so a Heavy with Microgun at 1800 px still engages but at low priority (PURSUE_PICKUP wins until the bot closes range).
  2. `tactic_engage`'s strafe uses `weapon_ideal_strafe_px` instead of the M6 P04 hardcoded 400 px (Rail Cannon now strafes at 1100 px).
  3. `nav_pick_position(BOT_POS_ENGAGE)` sub-scores by proximity to `optimal_range_px` and gets the `prefers_high` bias for Rail Cannon — engagement nodes 64+ px above the target score 1.30×.
  Iter9 30-sec matrix vs iter7 60-sec reference: Riot Cannon fires-per-kill 4 833 → 2 213 (-54 %); Microgun 615 → 456 (-26 %); Mass Driver kills 76 → 66 (-13 %, closer to median).

- **Phase 4 — Aggression-based retreat.** New `bot_aggression(world, mind, mid)` — 0..1 score from HP fraction (55 %), armor fraction (20 %), ammo fraction (10 %), damage recency (15 %), then × personality.aggression baseline. `BotMind` tracks `last_hp_observed` + `last_hurt_tick` (set by strategy-tick HP-delta detection). Score_engage applies: agg < 0.30 → 0.2× damp (commit to retreat); agg > 0.70 → 1.2× boost (push through pickup grabs). New `tactic_retreat` picks a `BOT_POS_COVER` node + paths there; aim stays on enemy so opportunistic-fire still triggers if LOS opens. New per-tier `retreat_threshold` in BotPersonality (Recruit 0.0 — never retreats, "bad on purpose"; Veteran 0.25; Elite/Champion 0.30).

- **Phase 5 — CTF team-role coordination.** New `BotTeamRole` enum (NONE/ATTACKER/DEFENDER/FLOATER/CARRIER) + `BotMind.team_role`. `bot_assign_team_roles(bs, world)` runs per-team round-robin: 2 attackers / 1 defender / 1 floater for 4-bot teams (mirrors Q3's default split). New `g_role_goal_mult[role][goal]` table multiplies every goal's score by a per-role weight before the winner is picked (Defender: GRAB_FLAG ×0.3, DEFEND_FLAG ×1.6, RETURN_FLAG ×1.4; Attacker: GRAB_FLAG ×1.5, DEFEND_FLAG ×0.3; Carrier: CAPTURE ×2.0, ENGAGE ×0.3). New `score_defend_flag` + `tactic_defend_flag`. Assigner runs at `start_round` (`main.c`) AND on flag-state transitions (`broadcast_flag_state_if_dirty`). Bake harness mirrored. Carrier role is dynamic — overrides whatever was assigned at round start whenever `ctf_is_carrier(w, mid)`.

- **Phase 6 — Mass Driver balance pass.** `src/weapons.c`: fire_rate_sec 1.10 → 1.25 (-12 %), aoe_radius 160 → 140 (-13 %). Plan §4.6 option C (50/50 split between fire-rate and AOE). Iter9 matrix: MD kills 76 → 66, closer to the 55 median; matrix-dominant entry retired.

- **Diagnostics + observability for future-Claude playtest sessions.** New `BOT_TRACE=1` env var prints per-strategy-tick goal picks (`t=X mid=N goal=engage tm=K tn=J pl=L score=S pos=...`). New `BOT_NAV_DUMP=1` (unused in production builds but reserved). New `tools/bake/run_1v1_sweep.sh` (Phase-2 acceptance script — iterates all 8 maps × 4 tiers as 1v1 fires/kills/verdict). New `tests/net/run_bot_playtest.sh` — paired host (with `--bots 4 --bot-tier elite`) + observing client; 15 assertions on log milestones (`bot_assign_team_roles` fires, no crashes, snapshot stream healthy). Added as `make test-bot-playtest`. Visibility build's wall-clock time is appended to the `bot_nav: built ...` LOG_I so the 30 ms budget breach surfaces in the log instead of silently regressing.

Results overview (iter9, 60s 1v1 at all tiers + 30s matrix at Champion):

- 8v8 bake all 8 maps PASS (unchanged from M6 P04 iter7).
- **1v1 PASS rate: 26/32 cells** (was 11/32 baseline — 2.4× improvement). The remaining 6 cells are tier-specific edge cases on otherwise-passing maps: Foundry Recruit/Veteran (low-tier awareness + mid-cover wall), Concourse Recruit/Veteran (low-tier awareness across wide atrium), Aurora Elite/Champion (high-tier engagement-node picker overshoots), Crossfire Champion (Phase-2 engagement-node picker prefers a high node out of reach).

**Map geometry changes for 1v1 playability** (cook_maps.c):
- **Reactor**: pillar's lower wall raised — bottom 3 tiles cut so bots walk THROUGH the pillar at floor level via a 96-px-tall archway. The pillar still reads as the contested center; the window slit at rows 30-31 keeps the sniper LOS. Flank ramps elevated 96 px above floor so they don't block the bowl-floor nav chain (climbers JET to the ramp's bottom edge from the floor).
- **Catwalk**: floor-to-cw1 ramps + slide-slopes elevated 96 px above floor. Ground game keeps its character; bots can traverse the floor without colliding with slope polys.
- **Crossfire**: CTF entry ramps elevated 96 px above floor; flank cover stubs shortened from 3 tiles to 2 tiles so a mech's head clears them while walking past (the 3-tile stub blocked all floor traversal mid-round).
- **Citadel**: castle outer slopes elevated 96 px; castle partition's floor-level passage cut (the comment in the cook code said "partition with floor-level passage" but the implementation was solid — bots were trapped in the flag room with no way to reach the east passage).

**Bot-side support for the geometry changes**:
- `tactic_path_follow` / `tactic_engage`'s path-follow code pulse `BTN_JET` whenever the next path node is ≥ 24 px above the bot's pelvis AND the bot is grounded. The fuel-hysteresis (10/40 %) gives a natural duty cycle, so the bot makes incremental progress up slopes that gravity-along-tangent friction would otherwise pin it on.
- WALK reach classifier accepts slopes up to `|dy| ≤ 1.2 × |dx|` (~50°) — adjacent-floor slope walks skip the body_corridor check because the corridor ray clips through the slope poly itself.
- A* JET cost bumped 1.4× → 4.0× of WALK cost so paths prefer floor walks when both options exist. Prevents bots from picking JET paths they can't physically execute.
- `bfs_closest_reachable` fallback when A* fails (goal in a disconnected component) — bot paths to the most goal-proximate reachable node instead of pushing into geometry.

Previously: **2026-05-13** (M6 P04 post-merge polish — lobby bot UX rewrite + fuel-aware jet). User-reported issues retired in that round:

- **"There is no way to add bots in the lobby; the Bot Tier button in host setup is greyed out"** — Resolved by moving all bot config from the host-setup screen to a dedicated row in the lobby (the host-setup screen's panel went 8 rows → 6 rows, back to mode/map/score/time/ff/buttons). Lobby second row holds `Tier: [< Veteran >]` cycle + `[+ Add Bot]` button + `N bots` count. Per-bot removal via a `Remove` button on each bot row (replaced the suppressed-for-bots Kick button). New `NET_MSG_LOBBY_ADD_BOT` (id 35) wire message carries the host-UI client's request to the in-process server; bot removal reuses `NET_MSG_LOBBY_KICK` (the server-side handler now branches on `is_bot` and removes directly without trying to disconnect a peer that doesn't exist).
- **"Tier change rewrites all bots' tiers; I want per-bot tiers"** — The previous `lobby_apply_bot_fill`-driven design assumed a single shared tier. Replaced with: pick tier → click Add → that ONE bot lands at that tier; existing bots untouched. Removed `start_round`'s "safety net" call to `lobby_apply_bot_fill` (it was rewriting tiers between rounds). Bot slots now persist across rounds with their original tier intact.
- **"Cycle button arrows overlap the label"** — `ui_cycle_button` label centering now carves out the arrow regions before centering; long labels (Tier: Veteran in 128 px) no longer overlap the `<` / `>`. Also gained directional hit regions: left quarter is the "left arrow zone" (-1 regardless of mouse button), right quarter is "right arrow zone" (+1 regardless), middle keeps the LMB-fwd / RMB-back convention.
- **"Cycle button should also work for the host-setup Map control"** — Map button on the host setup screen converted from `ui_button` to `ui_cycle_button`; LMB advances, RMB rewinds through the mode-compatible map registry. New `setup_step_map_for_mode` helper walks the registry in either direction.
- **"When bots are in the lobby, the post-round map vote doesn't fast-forward"** — Bots now auto-cast a random vote (uniform across the 3 cards) the instant the vote cards appear. New `lobby_vote_cast_bots(L, a, b, c, rng)` helper called from both `main.c::host_start_map_vote` and `shotmode.c::shot_host_start_map_vote`. The existing all-voted-fast-forward path (drops `summary_remaining` to 1 s when every active in-use slot has voted) now triggers as soon as the humans have all cast — pre-fix, matches with humans + bots waited the full ~10 s timer.
- **"Bots get stuck mashing JET when fuel is empty / hold JET across cycles without letting it refill"** — Bot motor now gates BTN_JET on fuel availability with 10 %–40 % hysteresis. New `BotMind.jet_locked_out` field flips true when `fuel_frac < 0.10` and clears when it climbs back ≥ 0.40 — the 30-point band keeps "press → empty → release → regen one tick → press" chatter out of the wire. Stuck-recovery (after `BOT_STUCK_TICKS` of no horizontal progress) now also respects the lockout, and after ~4 s of persistent stuck the bot flips its walking direction so it stops pushing the same wall.
- **Pre-existing latent bug** — `host_view = (g->net.role == NET_ROLE_SERVER)` in `lobby_screen_run` only fired on the headless dedicated binary, so the host UI (which is `NET_ROLE_CLIENT` + `slot.is_host=true` after wan-fixes-16) never showed Kick / Ban / Remove buttons in the player list. Changed to `host_view = is_host`. Made the M4 Kick/Ban feature actually functional on the host UI; without this fix the new "Remove bot" affordance wouldn't have rendered either.

Previously: **2026-05-13** (M6 P04 — map balance + bot AI hardening). Bake-test-driven iteration pass: ran a 320-cell sweep (8 maps × 5 chassis × 8 primaries, 60 s/cell at Champion tier) to measure what each loadout actually does on each map. Pre-iteration most maps produced 0 fires/kills regardless of tier; post-iteration all 8 maps PASS at Champion (every map produces real combat) and the loadout matrix shows distinct top performers per map. Full report at `documents/m6/04-map-balance.md`. Changes:
...
Previously: **2026-05-12** (M6 — CROUCH + PRONE poses implemented and network-synced). User-reported "crouch and prone don't appear to be working" — verified pre-implementation: `BTN_CROUCH` (S/Down) and `BTN_PRONE` (X) existed in `input.h` and `SNAP_STATE_CROUCH` / `SNAP_STATE_PRONE` existed in `snapshot.h`, but NOTHING consumed them. No `ANIM_CROUCH` / `ANIM_PRONE` in `AnimId`; no anim_id transition in `mech_step_drive`; no pose rules in `pose_compute`. Implementation:
  ...
  - `pose_compute`: CROUCH keeps the torso at full length (shortening it violated `PELVIS-L_SHOULDER` / `PELVIS-R_SHOULDER` cross-brace rest distances and produced a -0.28 px/tick backward drift), only bending the knees forward via IK with the foot target at chain*0.55 below the hip — leg chain at rest length, no constraint pulls. PRONE rotates the entire skeleton 90° around the pelvis (using `face_dir`), preserving every inter-bone distance — torso, shoulders, hips, knees, feet all keep their rest-length offsets from each other; the body extends horizontally along the ground.
  - `snapshot_capture` / `snapshot_apply`: populate `SNAP_STATE_CROUCH` / `SNAP_STATE_PRONE`; client decodes the bits and sets `anim_id` (priority: JET > PRONE > CROUCH > RUN > STAND).
  - `render.c::update_camera`: camera now follows the PELVIS (was CHEST). The chest is a pose-output that shifts forward in PRONE (causing the camera to drift forward while the body's actual position stayed put — perceived as "sliding backwards"). Pelvis is the stable anchor.

New visual tests: `tests/shots/m6_crouch_prone.shot` (single-mech cycle through STAND → CROUCH → STAND → PRONE → STAND), `tests/shots/m6_pose_drift.shot` (long-hold drift verification — CROUCH now ~+0.02 px/tick, was -0.28; PRONE ~+0.7 px/tick — see trade-off), `tests/shots/net/2p_crouch_prone.{host,client}.shot` (paired-shot sync verification on Aurora). Both views render matching CROUCH and PRONE poses for the remote mech.

Previously: **2026-05-12** (M6 P01 audit round 2 — slope pose sync between local and remote views).
  - **Mech pose desync on slopes** (user-reported): own-window showed the mech in a slope-adapted pose (feet on slope surface, body leaning), but other-client windows showed the same mech in a rigid straight-legged standing pose. Root cause: `physics_constrain_and_collide`'s tile / poly collision passes gate on `inv_mass > 0` (wan-fixes-3, so kinematic remote-mech particles aren't displaced). The LOCAL mech's bones were pushed out of the slope poly by that gate; pose_compute then wrote straight-down targets that the swept-test in `pose_write_to_particles` clipped to the slope surface. The REMOTE mech's bones never went through poly collision (inv_mass=0), so pose_compute wrote the same straight-down targets but they landed inside the slope poly with no push-out. Fix: new `physics_push_mech_out_of_terrain` (in `src/physics.c`, exposed via `physics.h`) runs AFTER `pose_write_to_particles` in `simulate_step` for EVERY alive mech, ignoring `inv_mass`. It does a one-pass kinematic push (updates pos AND prev — preserves velocity) out of any overlapping solid tile or polygon. Now remote-on-client bones get the same slope-surface clipping as local bones. Verified visually with `tests/shots/net/2p_slope_pose.{host,client}.shot` on Aurora's west hill.

Previously: **2026-05-12** (M6 P01 audit — three play-test regressions fixed). User-reported on first play-test:
  - **Gravity / jet / jump felt anemic** — root cause: `pose_write_to_particles` was setting `prev = pos` every tick per the doc's §7.9 pseudocode, which zeroed `(pos - prev) = velocity` each frame so gravity/jet/jump impulses couldn't accumulate. Fall rate plateaued at ~80 px/s instead of accelerating; jet was barely a bump. Fix: use `physics_translate_kinematic_swept` for each bone — translates pos AND prev by the same delta, preserving velocity AND swept-testing against tiles so deterministic bone offsets that would land inside a wall stop at the surface instead.
  - **Grapple "no stretchy rope feel" / instant pull** — root cause: `solve_fixed_anchor` (tile-anchor grapple constraint) applied 100% of the overshoot per relaxation iter; with 12 iters/tick the firer effectively snapped to rope length on tick 1. Fix: per-iter stiffness factor `GRAPPLE_ROPE_STIFFNESS = 0.10` (~72% per tick) — same factor also applied to `solve_distance_limit` (the bone-anchor grapple, only user of that solver). Rope now visibly stretches and recoils across a few ticks.
  - **Mech pulled through platforms during retract** — root cause: retract rate of 800 px/s = ~13 px/tick × hard constraint = pelvis dragged straight through any intervening geometry. Fix: retract slowed to 400 px/s; constraint pull `swept-tests` against the level (clamps the move just shy of the first solid tile it would cross); stretchy constraint gives the tile-collision pass more chances to push the pelvis out.

New shot tests: `tests/shots/m6_fall_test.shot` (1 mech, in-air drop to floor — visual gravity verification), `tests/shots/m6_jet_test.shot` (jet hold + release, visual thrust + fall), `tests/shots/m6_grapple_swing.shot` (Reactor pillar — fire grapple, pendulum swing, retract, disconnect, re-fire — matches the user spec "shoot a platform above your head, disconnect and shoot it again").

Previously: **2026-05-12** (M6 P01 — IK + pose sync, all phases landed). New `src/mech_ik.{c,h}` module ships a 2-bone analytic IK + a deterministic `pose_compute(PoseInputs, PoseBones)` that maps (pelvis, aim, facing, anim_id, gait_phase, grounded, chassis, active_slot, dismember_mask, foregrip, grapple) → 16 bone positions. Replaces `build_pose` + `apply_pose_to_particles` (Verlet constraint-solver pose driver) — `pose_compute` runs in `simulate.c` AFTER physics + `mech_post_physics_anchor`, and `pose_write_to_particles` kinematically writes pos AND prev for every non-dismembered particle (zero injected velocity). Dead and dismembered limbs stay on free-flying Verlet. New wire field `EntitySnapshot.gait_phase_q` (u16, between `ammo_secondary` and the optional grapple suffix; 1/65536 Q0.16 resolution); `ENTITY_SNAPSHOT_WIRE_BYTES` 29 → 31; protocol id bumped `S0LI` → `S0LJ`. `src/mech.c::build_pose` stripped down to `mech_update_gait` (anim_time increment + gait_phase_l/r update + footstep SFX wrap), gated to auth+local on the client; remote-mech footstep SFX wraps fire from `snapshot_apply` using the same plant-location math. Three concrete bugs retired: (A) remote mechs frozen in rest pose post-wan-fixes-3 — bones now animate from snapshot inputs; (B) non-hitscan self-fire SFX silent on the firer's window — `src/net.c::client_handle_fire_event` `predict_drew` split into `_sparks` (active-slot self regardless of fire kind) and `_sfx` (additionally requires `WFIRE_HITSCAN` since predict only plays SFX for that case); (C) procedural pose without a deterministic driver — bones now a pure function of synced inputs, no per-tick history feedback. New tests: `tests/mech_ik_test.c` (16 assertions on 2-bone IK math), `tests/pose_compute_test.c` (27 assertions on determinism + anim coverage + foregrip), `tests/snapshot_test.c` (+5 gait_phase round-trip assertions), `tests/shots/net/run_riot_cannon_sfx.sh` (3 paired-shot assertions). TRADE_OFFS.md retires "Remote mechs render in rest pose on the client (wan-fixes-3)" and "Left hand has no pose target (no IK) — incl. two-handed weapons (P11)"; adds "Live-mech bones are procedural; Verlet is for ragdoll only (M6)" and "Footstep SFX fires from two paths (M6)".

Previously: **2026-05-12** (post-P19 bug-fix #2 — kinematic remote mechs were stretching from a pose-drive feedback loop). The previous round's `apply_run_velocity` slope-Y fix wasn't enough: paired-window LAN test on Aurora still showed the bug, and a new paired-shot test on Aurora's west hill captured the root cause via SHOT_LOG bone-position dumps.

**What was happening.** On the host, the local mech's bones stayed at rest length the whole climb (chest_dy = -30 ±0.1 px, foot_dy = +36 ±gait swing) — the constraint solver pulled them back each tick. But on the client, the remote (host's) mech's `chest_dy` started at -30 at t=0 and drifted to **-364 by t=580** while the host was running. Same drift for head_dy. Bones inflating ~1-3 px per tick, compounding.

Root cause: `apply_pose_to_particles` was running on **kinematic remote mech particles** (inv_mass=0 per wan-fixes-3). `physics_translate_kinematic_swept` moves particles directly — it doesn't gate on inv_mass like the physics passes do. Combined with `build_pose`'s `pelvis_y_pose = foot_y_avg - chain_len` override (correct under the constraint solver, broken without it), this created a feedback loop in ANIM_RUN: gait swing lifts feet → foot_y_avg moves up → pose pelvis_y target moves up → chest target moves up → apply_pose lifts the chest → next tick's gait recomputes against a body that's already moved → bones inflate. The host's constraint solver pulled bones back each tick, masking the bug locally; remote mechs on the client lack that stabilizer because every particle has inv_mass=0 (every constraint short-circuits).

**Fix**: `apply_pose_to_particles` now early-returns for remote mechs on the client (`!w->authoritative && m->id != w->local_mech_id`). The body stays in whatever pose `mech_create_loadout` initialized it to (chassis rest pose), and `snapshot_interp_remotes` rigid-translates the whole thing along with the pelvis. Matches the existing `Remote mechs render in rest pose on the client` trade-off — pose drive on a kinematic body was always going to be either inert or unstable.

Verified: `tests/shots/net/run.sh 2p_slope_aurora` shows `chest_dy = -30.0` stable across 1200+ ticks (was drifting to -364 pre-fix). `tests/net/run.sh` 13/13 PASS. The previous `apply_run_velocity` Y-uniform fix from #1 stays — it solved the host-side stretch under heavy slope load.

Previously: **2026-05-12** (post-P19 bug-fix — mechs visibly grew while running uphill). User report: while walking UP a slope, both the host and joining mech inflated in size; reaching the peak restored normal size; walking BACKWARDS down the slope didn't trigger it. Root cause in `src/mech.c::apply_run_velocity`: when running on a slope, only the lower-chain particles (`PART_L_FOOT`, `PART_R_FOOT`, `PART_L_KNEE`, `PART_R_KNEE`) received the slope-tangent Y-velocity (`vt_per_tick_y`); the upper body (pelvis, chest, head, arms) got only the X component. The original comment claimed this asymmetry created a "natural lean into the slope." In practice it stretched the body: feet + knees climb ~2 px per tick (slope tangent goes up), upper body only moves horizontally → 2 px of vertical separation injected each tick. The 12-iteration constraint solver couldn't fully converge against a sustained per-tick injection on a long chain (feet → knees → thighs → pelvis → chest → head/arms), so the stretch accumulated across a held-run climb and rendered as a visibly growing mech. Going downhill didn't trigger it because gravity already pulled the upper body down at the same rate as the climb tangent (same net displacement, no stretch). Fix: apply the slope-tangent Y-velocity uniformly to every particle (1 line — drop the `if (foot/knee)` gate). Body translates rigidly along the tangent; the slight loss of "lean" is far less noticeable than the body inflating. `tests/net/run.sh` 13/13 PASS.

Previously: **2026-05-12** (P19 follow-up #7 — race in wan-fixes-3 left the JOINING client's own mech with permanently-zero inv_mass). After the follow-up #6 fixes shipped, paired-window LAN test surfaced a related bug: from the joining client's window, the local mech (the user's mech) didn't fire footsteps, while the remote (host) mech's did. Gunshots worked fine. Root cause: the wan-fixes-3 `inv_mass`-zeroing loop in `src/simulate.c:190` skips `i == local_id`, but `local_mech_id` resolves a few ticks AFTER the local mech is created by the snapshot stream (the lobby slot→mech mapping arrives on a separate net message). During that window `local_id == -1`, so `i == -1` is false for every valid index and the local mech's particles get zeroed alongside the remotes. Once `local_mech_id` finally resolves, the loop stops zeroing the local mech but never restores `inv_mass` to 1.0 — leaving prediction physics permanently dead on the joining client's own mech. Symptom matched exactly: gunshots fire fine (they don't read inv_mass), but `PARTICLE_FLAG_GROUNDED` is never set on the local mech's feet, so `any_foot_grounded` returns false and the new mech_step_drive grounded-gate (follow-up #6) silences footstep audio. Fix: rewrite the loop to set BOTH the local mech (1.0) AND remotes (0.0) every tick — idempotent and self-healing once local_mech_id resolves. `tests/net/run.sh` 13/13 PASS.

Previously: **2026-05-12** (P19 follow-up #6 — remote footsteps + per-client music). After paired-window LAN test, two structural audio bugs surfaced.

**Bug 1 — Remote enemies never produced footsteps for the observer.** Gunshots crossed the wire fine, but walking past another mech was silent. Root cause: `mech_step_drive` at `src/mech.c:982` unconditionally overwrote `m->grounded` with `any_foot_grounded(w, m)`, which reads `PARTICLE_FLAG_GROUNDED` from the particle pool. For remote mechs on the client, **wan-fixes-3** zeroed `inv_mass` on every remote particle to make them kinematic — the physics collision pass skips them, so `PARTICLE_FLAG_GROUNDED` is never set. Snapshot apply *did* deliver the correct `SNAP_STATE_GROUNDED` bit to `m->grounded`, but `mech_step_drive` immediately clobbered it with the locally-computed `false`. The swing→stance footstep trigger in `build_pose`'s `ANIM_RUN` case is gated on `m->grounded`, so remote footsteps were ALWAYS skipped. Fix: gate the `m->grounded = any_foot_grounded(...)` overwrite on `w->authoritative || mid == w->local_mech_id` — only the side that owns physics for that mech recomputes from contact flags; everyone else trusts the snapshot value.

**Bug 2 — Music only played on the host window.** `apply_audio_for_map` (which calls `audio_set_music_for_map` + `audio_music_play` + `audio_set_ambient_loop`) was a file-static in `main.c` and only called from the host's `start_round`. The client's `net.c::client_handle_round_start` rebuilds the level + initializes pickups + flags + decal layer but never touched audio. Fix: moved `apply_audio_for_map` into `src/audio.{c,h}` as the public `audio_apply_for_level(const Level *L)` (along with the helper `resolve_level_string_path` that decodes STRT-indexed paths to `assets/...` form). Both `start_round` and `client_handle_round_start` now call it. The function is idempotent on same-map round loops via `audio_set_music_for_map`'s path-dedup guard.

Verified: `tests/net/run.sh` — 13/13 PASS. Build clean. Audio smoke PASS.

Previously: **2026-05-12** (P19 follow-up #5 — `loudnorm` was destroying short SFX). User reported footsteps still inaudible after the servo was muted. Instrumented the gait swing→stance trigger with `SHOT_LOG`; confirmed the footstep call fires correctly (8 events over a 2-second walk at run cadence). So the trigger logic was fine. Then audited the actual sample loudness: the Kenney source `footstep_concrete_000.ogg` peaks at -0.9 dB (near full-scale), but the shipped `assets/sfx/footstep_concrete.wav` peaked at **-25.5 dB after our loudness pass** — a 24 dB drop. Root cause: `ffmpeg -af loudnorm=I=-16:TP=-1.5:LRA=11` measures *integrated* LUFS over the whole sample including the silent tail after a brief impact. For a 0.12 s footstep with a 0.05 s fade-out, the integration window averages near-zero, so loudnorm thinks the cue is quiet and pulls gain DOWN to hit -16 LUFS. Same destruction hit hit_flesh (-26.6 dB peak), explosion_large (-25.3 dB), jet_pulse, and others. Fix: **SFX (all WAVs) use peak normalization to -1.5 dB** instead of LUFS; only music + ambient OGGs keep LUFS-based loudnorm (long sustained content where integrated loudness is meaningful). After the fix, every SFX peaks at -1.5 dB and the per-slot `volume` field in `g_sfx_manifest` is what differentiates relative loudness (mass_driver 1.00 / pulse_rifle 0.85 / hit_flesh 0.90 / footstep 0.70 / ui_click 0.65 etc.). Footsteps should now be ~29 dB louder than they were — clearly audible from the local mech and remote enemies in the spatialized 200–1500 px attenuation range. Confirmed by SHOT_LOG that footsteps fire on every mech `simulate_step` walks (line 206) including remote-on-client, so opponents' footsteps reach `audio_play_at` for spatialized playback.

Previously: **2026-05-12** (P19 follow-up #4 — servo disabled, footsteps amplified for competitive listening). After three rounds of trying to fix the servo loop's character (`spaceEngine` → `forceField` → `computerNoise`), the user concluded the continuous mech-presence hum itself was the problem regardless of source: it masked the per-plant footstep cue and made the walking sound feel "annoying." Resolution: **`SERVO_MAX_VOL = 0.0f`** in `src/audio.c` — the runtime path stays wired (sample still on disk, `audio_servo_update` still ticks) but produces no audible output. The footstep is now the sole walking cue. To compensate, **footstep volume bumped 0.40 → 0.70** so enemy footsteps at mid-range (~500–800 px) are clearly audible — the SFX bus's distance attenuation in `audio_play_at` (linear 1.0 → 0 from 200 → 1500 px) does the spatial work; the manifest gain controls intelligibility. Both `mech_step_drive` and `build_pose` run on every mech (local + remote) per `simulate.c::simulate_step` line 206 and remote mechs receive `SNAP_STATE_GROUNDED` + `SNAP_STATE_RUNNING` from snapshots — so footstep events fire on the client for remote enemies just like they do for the local mech, which gives the competitive-listening behavior the user asked for. To re-enable the servo later, raise `SERVO_MAX_VOL` to e.g. 0.10.

Previously: **2026-05-12** (P19 follow-up #3 — servo loop rework after the "walking sound is whwoo whwoo" report). The user perceived the SERVO LOOP — not the footsteps — as the dominant walking cue, and called the pitch modulation in `spaceEngine_000.ogg` annoying ("pulsing whwoo whwoo, continues to play when round ends even though no one is moving"). Three changes:

- **Servo source swapped** twice: `spaceEngine_000.ogg` (5 s pitch-modulated engine drone) → `forceField_002.ogg` (0.95 s — too short, retriggered ~1 Hz which created its own pulse rhythm) → `computerNoise_001.ogg` (5 s steady electronic hum without internal modulation, trimmed to 3 s with 50 ms fade-in + fade-out at both seams). New `mkwav_loop` helper in `source_map.sh` is the looper-safe variant of `mkwav` — adds the start-fade so retrigger boundaries don't click.
- **Servo volume + decay** in `audio.c`: `SERVO_MAX_VOL` 0.7 → **0.20** (subliminal background, not "the walking sound"), `SERVO_LERP` 0.15 → **0.30** (~167 ms fade vs ~500 ms). Added a hard cutoff in `audio_servo_update`: when `target_vol == 0` and `current_vol < 0.04`, snap to 0 and `StopSound` immediately — the user reported the 500 ms tail was audible after round-end; the cutoff guarantees silence within ~100 ms of motion stop, regardless of where in the decay curve we are.
- **Footstep volume** bumped 0.25 → **0.40** so the discrete per-plant cue is now the dominant walking sound instead of being drowned by the servo. Footstep sources weren't changed (`footstep_concrete_000.ogg` is an actual concrete-footstep recording; the user's "doesn't sound like a footstep" complaint pointed at the servo, which they read as the walking cue).

Previously: **2026-05-12** (P19 follow-up #2 — mute / footstep fixes after paired-window LAN test). Three bugs surfaced when the user ran two soldut windows on localhost (Direct Connect 127.0.0.1):

- **Music didn't resume after unmute.** `SetMusicVolume(0)` is not a reliable mute path across raylib versions — some leave the stream playing-but-silent (correct), some halt decoding entirely. The latter meant unmuting set volume back to 0.30 but the stream was no longer advancing. Fix: `audio_toggle_mute` now calls `PauseMusicStream` on mute and `ResumeMusicStream` on unmute. `music_step` early-returns while muted so we don't tick `UpdateMusicStream` against a paused stream.
- **Ambient + servo loops retriggered at volume 0.** `ambient_step` / `audio_servo_update` retrigger the underlying `Sound` whenever it stops playing. While muted, the retrigger fired every frame against a 0-volume sound — wasteful and a potential source of clicks. Fix: `audio_toggle_mute` `StopSound`s both on mute + zeroes `g_servo_current_vol`; `ambient_step` and `audio_servo_update` early-return on `g_muted` so the unmute frame is the one that restarts them fresh.
- **Footsteps too loud.** Sample volume was 0.55, and the metal/ice samples were 0.37 s long — at run cadence (3-4 plants/sec) consecutive footsteps overlapped into a "thunk-thunk" drone. Fix: footsteps in `g_sfx_manifest` dropped to 0.25, sources trimmed to 0.12 s (metal/concrete) / 0.18 s (ice) via `source_map.sh::mkwav` max-duration args. Same overlap-stacking fix pattern as the post-P19 jet/launcher trim.

**Known non-fix**: inter-window music drift. Two soldut processes each play their own music stream — when both start the same map, the streams begin at slightly different times and drift further with playback. The user perceived this as "music stopped" after muting one window because the doubled (slightly phase-shifted) layer was gone. Real fix would require cross-process music sync, which is out of scope; F2 in each window you want silent is the workflow.

Previously: **2026-05-12** (P19 follow-up — F2 mute + SFX trim). Two small post-P19 polish items: (a) `mass_driver` / `micro_rockets` / `jet_pulse` / `jet_boost` source samples were full-length thruster/engine recordings (4-5 s each) — the runtime played them at full duration which produced continuous-rumble stacking when the player held jet or fired the launchers. `tools/audio_inventory/source_map.sh::mkwav` now takes an optional max-duration arg that maps to `ffmpeg -t <s> -af afade=t=out:...`, and the long-source slots got per-slot caps: jet_pulse 0.15 s (pulse rate is 67 ms — needs to clear before the next), jet_boost 0.60 s, mass_driver 0.40 s, micro_rockets 0.30 s. The brief's other long-source slots (explosions, kill_fanfare, some pickups) were also tightened. Re-sourced + loudness-normalized in one pass. Total assets now 6.9 MB (down from 7.9 MB after trimming). (b) **F2 toggles a global mute** for the running process — `audio_toggle_mute` / `audio_is_muted` on `src/audio.{c,h}`, `bus_resolved` short-circuits to 0 when set so SFX + UI + music + ambient + servo all silence instantly without disturbing per-bus gains. F2 keypress edge-detected at the top of `main.c`'s frame loop (works in title / lobby / browser / host-setup / connect / match / summary). New `audio_draw_mute_overlay(sw, sh)` paints a top-center "F2  MUTED" pill when active, wired into `renderer_draw_frame` (both halftone + fallback branches) + all 6 non-match `EndDrawing` sites in `main.c`. Lobby UI's Controls modal gained a "F2 — Mute audio" row. Intended use: paired-process LAN net tests where two soldut windows on the same machine stack their audio.

Previously: **2026-05-12** (P19 — audio assets fill). Closes M5. The P14 audio runtime now boots clean: 47 SFX one-shots under `assets/sfx/*.wav` + a `mech_servo_loop.wav` + 7 per-map music tracks (`assets/music/<kit>.ogg`) + 7 ambient loops (`assets/sfx/ambient_<kit>.ogg`) — 62 files, 7.9 MB total, well under the 30 MB budget from `documents/m5/09-audio.md` §"Performance budget". `audio_init` logs `47/47 samples loaded`. Sources: **Kenney CC0 audio packs** (`sci-fi-sounds` / `impact-sounds` / `digital-audio` / `ui-audio` / `interface-sounds` / `music-jingles`) for every SFX one-shot + servo + ambient loop; **opengameart.org CC0** (ruskerdax, Nicole Marie T, Joth, et al.) for the 7 music tracks. New tooling: `tools/audio_inventory/run_inventory.c` + `make audio-inventory` (CSV walk of `audio_manifest_*` accessors I added to `src/audio.{c,h}`); `tools/audio_inventory/source_map.sh` (the canonical "which Kenney/OGA source maps to which manifest slot" record — re-pinning a sound = edit + re-run); `tools/audio_normalize/normalize.sh` + `make audio-normalize` (idempotent ffmpeg pass to 22050 Hz mono PCM16 WAV / mono Vorbis q2-q4 OGG; `ARGS=--loudness` adds -16/-23/-28 LUFS loudnorm for SFX/music/ambient); `tools/audio_inventory/check_credits.sh` + `make audio-credits` (gates CC0 hygiene — every shipped audio file must appear in `assets/credits.txt`); `tests/audio_smoke.sh` + `make test-audio-smoke` (boots a 12-tick scripted run on Foundry and FAILs if any `audio:.*missing` line appears). New on-disk runtime state: `assets/sfx/` (55 files: 47 SFX + 1 servo + 7 ambient), `assets/music/` (7 tracks). Raw download artifacts (Kenney ZIPs + extracted Audio/ trees + OGA mp3/ogg downloads) sit under `assets/raw/audio/` and are gitignored — re-derivable from the URLs in `assets/credits.txt` + `tools/audio_inventory/SHOPPING.md`. `audio.h` gained four accessor functions (`audio_manifest_count`, `_path`, `_kind`, `audio_servo_path`) so the inventory tool walks `src/audio.c::g_sfx_manifest` as source-of-truth without duplicating it. `TRADE_OFFS.md`: "SFX manifest assets aren't on disk yet (post-P14)" **deleted**. **No new P19 trade-off entries** — the multi-variant footstep slots noted in the P14 spec doc were never wired into the runtime manifest (single slot per surface, with 5-alias rotation handling the per-fire variation), so what the runtime asks for is what shipped. Aurora music swapped from "Blue Ants" (17 s; too short to feel like a track inside a round) to Joth's "Cyberpunk Moonlight Sonata v2" (1:57) — the substitution is recorded in `assets/credits.txt`. With P19 in, M5 is complete: every system has its content, every map plays with sound, and the bake-test heatmaps from P18 are the next-milestone iteration starting point.

Previously: **2026-05-11** (P17 follow-up — two cross-build sync fixes shipped on top of the cook_maps tool after a user-reported "first round maps not synced" repro. Headline deltas:
